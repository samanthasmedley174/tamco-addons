...
local updateControlFunctions =
{
    [LibHarvensAddonSettings.ST_CHECKBOX] = function (self, control)
        ...
        local function toggle(targetControl, state)
            if state then
                targetControl:GetNamedChild("On"):SetColor(ZO_DEFAULT_ENABLED_COLOR:UnpackRGB())
                targetControl:GetNamedChild("Off"):SetColor(ZO_DEFAULT_DISABLED_COLOR:UnpackRGB())
                ZO_CheckButton_SetChecked(targetControl.checkbox)
            else
                targetControl:GetNamedChild("On"):SetColor(ZO_DEFAULT_DISABLED_COLOR:UnpackRGB())
                targetControl:GetNamedChild("Off"):SetColor(ZO_DEFAULT_ENABLED_COLOR:UnpackRGB())
                ZO_CheckButton_SetUnchecked(targetControl.checkbox)
            end
        end
        ...
        ZO_CheckButton_SetToggleFunction(
            checkbox,
            function (checkboxControl, state)
                toggle(checkboxControl:GetParent(), state)
    ...
    [LibHarvensAddonSettings.ST_SLIDER] = function (self, control)
        ...
        slider:SetHandler(
            "OnValueChanged",
            function (sliderControl, sliderValue)
                local formattedValue = tonumber(string.format(self.format, sliderValue))
                if self.unit and #self.unit > 0 then
                    sliderControl.label:SetText(formattedValue .. self:GetString(self:GetValueOrCallback(self.unit)))
                else
                    sliderControl.label:SetText(formattedValue)
                end
...
-----
--- @class Settings_ParametricList : ZO_Gamepad_ParametricList_Screen
...
function LibHarvensAddonSettings:CreateControlPools()
    ...
    AddPool(
        ...
        function (control)
            ...
            checkbox.SetText = function (controlReceivingText, text)
            end
    ...
    AddPool(
        ...
        function (control)
            ...
            function control:SetValue(...)
                local pooledSliderControl = self.slider
                pooledSliderControl:SetValue(...)
            end
        end
    )
    ...
    AddPool(
        ...
        function (control)
            ...
            function control:SetValue(value)
                local valueEditBox = self.editBox
