...
local function getAvgPrice(itemLink)
    -- Since TSCPriceDataAPIXBNA is a library dependency, it will always be available
    if not TSCPriceDataAPIXBNA then
        return nil
    end
    return TSCPriceDataAPIXBNA:GetPrice(itemLink) -- Returns nil if no data found
end
...
local function getFormattedPriceRange(itemLink)
    ...
    local itemData = TSCPriceDataAPIXBNA:GetItemData(itemLink)
...
local function initializeSavedVars()
    ...
    TSC.savedVars = ZO_SavedVars:NewAccountWide(
        "TSCPriceFetcherDataXBNA", SAVED_VARS_VERSION, nil, TSC.default)
