...
local EXPECTED_PLATFORM = 3  -- 0 = XBNA, 3 = XBEU, 1 = PSNA, 4 = PSEU
...
local function getAvgPrice(itemLink)
    -- Since TSCPriceDataAPIXBEU is a library dependency, it will always be available
    if not TSCPriceDataAPIXBEU then
        return nil
    end
    return TSCPriceDataAPIXBEU:GetPrice(itemLink) -- Returns nil if no data found
end
...
local function getFormattedPriceRange(itemLink)
    ...
    local itemData = TSCPriceDataAPIXBEU:GetItemData(itemLink)
...
local function initializeSavedVars()
    ...
    TSC.savedVars = ZO_SavedVars:NewAccountWide(
        "TSCPriceFetcherDataXBEU", SAVED_VARS_VERSION, nil, TSC.default)
...
local function setupSettingsMenu()
    ...
    local whatsNewButton = {
        type = LHAS.ST_BUTTON,
        label = "What's New",
        tooltip = [[v142: Updated Updated with new sales data for week starting 2025-12-02.
