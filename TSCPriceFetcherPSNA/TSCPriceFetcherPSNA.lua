-- TSCPriceFetcherPSNA.lua - Consolidated single file
local TSC = {
    name = "TSCPriceFetcherPSNA",
    version = 143
}
...
local EXPECTED_PLATFORM = 1  -- 0 = XBNA, 3 = XBEU, 1 = PSNA, 4 = PSEU
...
local function getAvgPrice(itemLink)
    -- Since TSCPriceDataAPIPSNA is a library dependency, it will always be available
    if not TSCPriceDataAPIPSNA then
        return nil
    end
    return TSCPriceDataAPIPSNA:GetPrice(itemLink) -- Returns nil if no data found
end
...
local function getFormattedPriceRange(itemLink)
    ...
    local itemData = TSCPriceDataAPIPSNA:GetItemData(itemLink)
...
local function initializeSavedVars()
    ...
    TSC.savedVars = ZO_SavedVars:NewAccountWide(
        "TSCPriceFetcherDataPSNA", SAVED_VARS_VERSION, nil, TSC.default)
...
local function setupSettingsMenu()
    ...
    local whatsNewButton = {
        type = LHAS.ST_BUTTON,
        label = "What's New",
        tooltip = [[v143: Updated with new sales data for week starting 2025-12-02.
